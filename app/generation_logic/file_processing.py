...
async def _extract_single_file(
    filename: str, request_id: str, file_content_bytes: bytes
...
async def _validate_and_extract_files(
    ...
    total_size = 0
    validated_files_data: List[Tuple[str, bytes]] = (
        []
    )  # To store (filename, content_bytes)
    ...
    # --- Total Size Check ---
    if (
        not validated_files_data and files
    ):  # No files were validated, but files were provided
        logger.warning(
    ...
    if total_size > MAX_TOTAL_SIZE:
        ...
        raise HTTPException(
            status_code=413,
            detail=f"La dimensione totale dei file ({total_size // (1024*1024)}MB) supera il limite di {MAX_TOTAL_SIZE // (1024*1024)}MB.",
        )
    ...
    tasks = [
        _extract_single_file(fname, request_id, f_content_bytes)
        for fname, f_content_bytes in validated_files_data
    ]
    ...
    for result_idx, result_item in enumerate(results):
        ...
        if isinstance(result_item, Exception):
            ...
            if isinstance(result_item, (ExtractorError, PipelineError)):
                ...
                raise PipelineError(
                    f"Error processing file '{original_filename}': {str(result_item)}"
                ) from result_item
        ...
        txt, tok = result_item  # result_item is Tuple[Optional[str], Optional[str]]
        if (
            txt is not None
        ):  # Ensure not None, could also check for non-empty string if needed
